...
# Evaluate PLM-stream cascade ablations on DUD-E HXK4 using an existing checkpoint.
...
OUT_DIR="${OUT_DIR:-${PROT_ROOT}/results/cascade_ablation_hxk4}"
...
"${PYTHON_CMD[@]}" "$SCRIPT_DIR/run_cascade_ablation.py" \
  ...
  --eval-mode test \
  --virtual-screening \
  --variants full drop_prottrans drop_esm1b_prottrans drop_all_plm \
  --out-dir "$OUT_DIR"
