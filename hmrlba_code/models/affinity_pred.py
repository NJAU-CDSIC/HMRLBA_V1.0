...
class AffinityPred(nn.Module):
    ...
    def _build_layers(self):
        ...
        if not self.toggles.get("ligand_only", None):
            ...
            if prot_mode == 'surface2backbone':
                bconfig = mpn_config['bconfig']
                sconfig = mpn_config['sconfig']

                if config.get("jk_pool", None) == "concat":
                    multiplier = bconfig['depth']
                in_dim += (multiplier * (bconfig['hsize'] + sconfig['hsize']))
    ...
    def forward(self, complex_data: ComplexData) -> torch.Tensor:
        ...
        s1_x = prot.surface.x
        s2_x = prot2.surface.x
        s3_x = prot3.surface.x

        concat_s_x = torch.cat((s1_x, s2_x, s3_x), dim=-1)
        # batch_size, num_features = concat_s_x.size()
        # new_num_features = num_features // 3
        # concat_s_x = torch.sum(concat_s_x.view(batch_size, 3, new_num_features), dim=1)

        b1_x = prot.backbone.x
        b2_x = prot2.backbone.x
        b3_x = prot3.backbone.x

        concat_b_x = torch.cat((b1_x, b2_x, b3_x), dim=-1)
        # batch_size, num_features = concat_b_x.size()
        # new_num_features = num_features // 3
        # concat_b_x = torch.sum(concat_b_x.view(batch_size, 3, new_num_features), dim=1)

        prot.surface.x = concat_s_x
        prot.backbone.x = concat_b_x
