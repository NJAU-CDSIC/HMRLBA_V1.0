...
from hmrlba_code.feat import SECONDARY_STRUCTS, AMINO_ACIDS, ATOM_LIST
from hmrlba_code.feat import IMP_VALENCE, EXP_VALENCE, DEGREES, ATOM_FDIM
from hmrlba_code.feat import BOND_FDIM, BOND_TYPES
...
class ResidueProp(object):
    ...
    def _set_bfactor(self, residue: Residue):
        atoms = residue.get_unpacked_list()
        # 计算 B-factor 的总和
        bfactor_sum = sum(atom.get_bfactor() for atom in atoms)
        # 计算原子数量
        num_atoms = len(atoms)

        # 计算平均 B-factor
        if num_atoms > 0:
            average_bfactor = bfactor_sum / num_atoms
            self.bfactor = average_bfactor
        else:
            self.bfactor = 0  # 避免除以零的情况
