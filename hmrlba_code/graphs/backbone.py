...
from hmrlba_code.feat import KD_SCALE
from hmrlba_code.feat.complex import (ResidueProp, get_residue_features,
...
class Backbone:
    ...
    @staticmethod
    def add_node_feats(amino_data: Data, residues: List, dssp_dict: Dict, emb: List) -> Data:
        x = []
        ...
        for idx, res in enumerate(residues):
            ...
            phi = dssp_dict[res.get_full_id()[2:]][3]
            psi = dssp_dict[res.get_full_id()[2:]][4]

            res_prop = ResidueProp(residue=res, sas=sas[idx], sec=ss, phi=phi, psi=psi,
                                   hydrophobicity=hydrophob, emb=emb[idx])
    ...
    def build(self, pdb_file: str, emb: List, target: Union[torch.Tensor, np.ndarray] = None, **kwargs) -> Data:
        ...
        Backbone.add_node_feats(amino_data, residues, dssp_dict, emb)
