...
from hmrlba_code.utils.surface import get_surface, compute_normal
from hmrlba_code.utils.mesh import (get_edge_points, set_edge_lengths,
...
class Surface:
    ...
    def build(self, pdb_file: str, emb: List, kdtree: KDTree, target: Union[float, np.ndarray] = None, **kwargs):
        import pymesh
        ...
        for vertex in mesh.vertices:
            _, nearest_idx = kdtree.query(vertex)
            closest_idxs.append(nearest_idx)

        node_feats = [emb[i] for i in closest_idxs]
